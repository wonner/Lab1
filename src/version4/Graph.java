...
public class Graph {
  ...
  public String[] pathString(int source, int end, ArrayList<ArrayList<Integer>> path) // 输出最短路径
  {
    ...
    for (ArrayList<Integer> p : path) {
      ...
      sb.append("The path from " + "\"" + digraph.get(source).vertexNode + "\"" + " to " + "\""
          + digraph.get(end).vertexNode + "\"" + ":");
  ...
  /*
   * public String randomWalk() { Random rand = new Random(); int start =
   * rand.nextInt(verNum-1); int end; LinkedList<Edge> eList =
   * digraph.get(start).edgeList; ArrayList<Integer> p = new ArrayList
   * <Integer>(); Boolean f = false; Scanner in = new Scanner(System.in);
   * 
   * while( eList.size()> 0 && !f) { System.out.println("输入0继续，1终止："); String
   * str=in.nextLine(); if(str.equals("0")) { int temp =
   * rand.nextInt(eList.size()); end = eList.get(temp).edgeNode; if
   * (find(p,end,start)) f = true;
   * System.out.println(digraph.get(end).vertexNode); p.add(end); start = end;
   * eList = digraph.get(start).edgeList; } else if(str.equals("1")) { break; } }
   * 
   * System.out.println("Random path:"); StringBuilder sb = new StringBuilder();
  ...
  public int randomWalk(int i) // 返回-1发现重复，停止，， 传入-1，则是第一个点，不进行比较；
  {
    ...
    if (i == -1)
      ...
      if (lastLoc == -1)
        ...
        thisLoc = digraph.get(lastLoc).edgeList.get(i).edgeNode;
